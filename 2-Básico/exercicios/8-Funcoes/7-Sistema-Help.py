...
while True:
    # Título inicial
    print(amend_color('', bg='green', color='white', style='bold', close_after=False), end='')
    titulo('SISTEMA DE AJUDA PyHELP', character='~')
    print(amend_color(''), end='')

    # Pegando o comando - 'fim' finaliza
    comando = str(input('Função ou Biblioteca: ')).strip().lower()
    if comando == 'fim':
        break

    # Titulo acessando
    print(amend_color('', bg='blue', color='white', style='bold', close_after=False), end='')
    titulo(f'Acessando manual do comando "{comando}"', character='~')
    print(amend_color(''), end='')
    # Conteúdo do comando
    print(amend_color('', color='white', style='negative', close_after=False), end='')
    help(comando)
    print(amend_color(''), end='')

# Finalizado
print(amend_color('', bg='red', color='white', style='bold', close_after=False), end='')
titulo('Até logo!!!', character='~')
print(amend_color(''), end='')
